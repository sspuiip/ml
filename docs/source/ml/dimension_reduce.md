...
&emsp;&emsp;**首先考查协方差矩阵$m\pmb{C}\triangleq\pmb{X}^\top\pmb{X}$与核矩阵$\pmb{K}\triangleq\pmb{XX}^\top$特征向量之间的关系**。
...
&emsp;&emsp;如果数据样本$\pmb{x}_i$与$\pmb{x}_j$很相似，则在拉普拉斯变换后的子空间与原空间一样，尽可能的接近。即，
...
&emsp;&emsp;使用条件概率$p_{j|i}$来评价数据点$\pmb{x}_i,\pmb{x}_j$的相似性。高维空间的距离可以通过**高斯分布**进行近似计算，
...
&emsp;&emsp;针对SNE的不对称问题，可以在低维和高维空间构造联合分布，使之任意$i,j$都有$p_{ij}=p_{ji}, q_{ij}=q_{ji}$。在低维空间构造，

$$
q_{ij}=\frac{\exp(-\Vert \pmb{y}_i-\pmb{y}_j\Vert^2)}{\sum_{k\neq l} \exp(-\Vert \pmb{y}_k-\pmb{y}_l\Vert^2}
$$

但在高维空间，按$q_{ij}$的形式构造$p_{ij}$则会存在问题。如果有一个离群点存在，则会造成离群点$i$与其余点$j$的$p_{ij}$都比较小。这就意味着，无论离群点映射在低维空间的任何位置，惩罚值都不会太大。所以需要采用其它方法。以下是一种简单直观的方法，既满足对称性又符合离群点的惩罚值不会过小。
...
&emsp;&emsp;虽然梯度计算简化了也解决了不对称问题，但效果只是略优于SNE,拥挤问题仍未解决。
...
从图中可以看出，高维空间较近的点，为满足$p_{ij}=q_{ij}$，在低维空间相同的$q_{ij}$，距离比高维空间更近。高维空间较远的点，为满足$p_{ij}=q_{ij}$，在低维空间相同的$q_{ij}$，距离比高维空间更远。换句话说，映射至低维空间后，高维空间较远的点，在低维空间距离更远；高维空间较近的点，低维空间距离更近。

&emsp;&emsp;引入$t$分布。在高维空间使用高斯分布，在低维空间使用$t$分布。可以使得高维空间较近的点在低维空间距离更近，高维空间较远的点在低维空间距离更远。这就达到了同一类簇内的点聚合的更紧密，不同类簇之间的点更加疏远的目标。在低维空间定义自由度为1的$t$分布重新定义$q_{ij}$，

$$
q_{ij}=\frac{(1+\Vert \pmb{y}_i-\pmb{y}_j\Vert^2)^{-1}}{\sum_{k\neq l}(1+\Vert \pmb{y}_k-\pmb{y}_l\Vert^2)^{-1}}
$$

仍然使用KL散度来衡量高维空间正态分布与低维空间$t$分布之间的差异程度，此时，梯度变为，

$$
\frac{\partial \mathcal{L}}{\pmb{y}_i}=4\sum_{j}(p_{ij}-q_{ij})(1+\Vert \pmb{y}_i-\pmb{y}_j\Vert^2)^{-1}
$$

&emsp;&emsp;仍然使用随机梯度下降法训练，即可得到$\hat{\pmb{Y}}$。这就是t-SNE算法。原文参考([Stochastic Neighbor Embedding](https://papers.nips.cc/paper/2002/file/6150ccc6069bea6b5716254057a194ef-Paper.pdf),[Visualizing Data using t-SNE](https://www.jmlr.org/papers/volume9/vandermaaten08a/vandermaaten08a.pdf)).
